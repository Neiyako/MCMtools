...
**这个 App 是自包含的**：`core/`、`templates/`、`web/` 都拷进了
`Contents/Resources/`，所以它可以被单独移动或拷贝到别的目录，
不需要旁边的仓库也能跑。模板、面板都从包里读。

签名是 ad-hoc（本机）的。拷到**另一台机器**时 Gatekeeper 会再拦一次，
用上面同样的办法放行即可，不必重新构建。

启动时用哪一份代码，顺序是：

1. 环境变量 `MCMTOOLS_HOME` 指到的仓库（如果你在用开发仓库）
2. App 包内的 `Contents/Resources/`
3. 由 `MCMtools.app` 所在位置上溯到的仓库

也就是说：**仓库还在原处时，App 跑的是仓库那份**（改了 `core/`、
`web/` 下次启动就生效，不用重建）；把 App 单独拷走时，自动退回包里那份。

想知道当前实际用的是哪条路径，看面板的
**设置 → 安装与路径**。那里显示的就是程序真正在用的值，
不是另算的一份 —— 模板"不见了"时先看这里。
...
| 面板里模板数是 0 / 找不到模板 | 路径没指对 | 看 **设置 → 安装与路径**，那里报的就是实际在用的路径 |
...
- **别指望拷 `MCMtools.app` 到新机器就能免检** —— ad-hoc 签名会被 Gatekeeper 拦一次，按第一次双击的办法放行即可（App 本身自包含，不用重建）
